...
export function handleStyleAttribute(
	...
	for (const [property, value] of Object.entries(styles) as [
		...
		// Si es una señal reactiva, se suscribe y actualiza cuando cambia.
			...
			const apply = (v: StyleValue) => applyStyle(element, property, v);
			const unsubscribe = value.subscribe((v) => {
				if (v) applyStyle(element, property, v as StyleValue);
			});
			apply(value() as StyleValue);
		...
		// Si es un arreglo, puede contener valores estáticos y/o señales.
			...
			const update = () => {
				...
				applyStyle(element, property, valueStr as StyleValue);
			};
